...
def calculate_disorder(people: list[int]) -> list[int]:
    """Calculates the disordering of people

    Args:
        people (list[int]): the positions of the people after bribes

    Returns:
        list[int]: the disordering of people
    """
    return [value - (index + 1) for index, value in enumerate(people)]


def find_first_negative(disorder: list[int]) -> int:
    """Find the first negative element in disorder array

    Args:
        disorder (list[int]): the disorder of the people positions

    Returns:
        int: the index of first negative element or -1 if nothing
    """
    return next(
        (index for index, value in enumerate(disorder) if value < 0), -1
    )


def calculates_bribe(
    people: list[int], disorder: list[int]
) -> tuple[list[int], int, int]:
    """Calculates the bribe
    ...
    Returns:
        tuple[list[int], int, int]: tuple of people reordered,
        person who changed position and the bribe cost
    """
    index = find_first_negative(disorder)
    if index - 2 >= 0 and disorder[index - 2] >= 2:
        people[index - 2], people[index] = people[index], people[index - 2]
        return (people, people[index], 2)
    if index - 1 >= 0 and disorder[index - 1] >= 1:
        people[index - 1], people[index] = people[index], people[index - 1]
        return (people, people[index], 1)
    return (people, -1, 0)


def minimum_bribes(people: list[int]) -> None:
    """New Year Chaos - Minimum Bribes

    Args:
        number (str): a string representation of an integer
        positions (list[int]): the positions of the people after all bribes

    Constraints:
        1 <= positions <= 10 ** 5
    """
    bribes_per_person = {value: 0 for value in people}

    disorder = calculate_disorder(people)

    while any(position != 0 for position in disorder):
        if any(bribes > 2 for bribes in bribes_per_person.values()):
            break
        people, person, bribe = calculates_bribe(people, disorder)
        if person < 0 or bribes_per_person[person] > 1:
            break
        bribes_per_person[person] += bribe
        disorder = calculate_disorder(people)

    else:
        print(sum(bribes for bribes in bribes_per_person.values()))
        return None
...
if __name__ == "__main__":
    ...
    minimum_bribes(people)  # prints 845 != 966 (expected)
